...
module.exports = __toCommonJS(jdate_exports);
...
var nameList = /* @__PURE__ */ __name((entries) => ({
  check(key, value) {
...
var flag = /* @__PURE__ */ __name(() => ({
  check(key, value) {
    if (typeof value !== "boolean") {
      throw new Error(`JDate config: "${key}" must be a boolean`);
    }
  },
  copy: /* @__PURE__ */ __name((value) => value, "copy")
}), "flag");
var CONFIG_SHAPE = {
...
function resolveConfig(base, overrides) {
  ...
  KNOWN_KEYS.forEach((key) => {
    resolved[key] = hasOwn(overrides, key) ? CONFIG_SHAPE[key].copy(overrides[key]) : base[key];
  });
...
var numeric = /* @__PURE__ */ __name((resolve) => (date, config) => {
...
var _JDate = class _JDate {
  ...
  constructor(...args) {
    ...
    const overrides = isPlainObject(last) ? dateArgs.pop() : void 0;
    if ((last === void 0 || last === null) && (dateArgs.length === 2 || dateArgs.length === 4)) {
      dateArgs.pop();
    }
...
//# sourceMappingURL=jdate.cjs.map
