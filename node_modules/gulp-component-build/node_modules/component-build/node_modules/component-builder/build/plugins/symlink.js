...
module.exports = function () {
  return wrapGenerator.mark(function symlink(file) {
    var out, link;

    return wrapGenerator(function symlink$($ctx0) {
      while (1) switch ($ctx0.prev = $ctx0.next) {
      case 0:
        return $ctx0.delegateYield(utils.exists(file.filename), "t0", 1);
      case 1:
        out = join(this.destination, utils.rewriteUrl(file.branch), file.path);
        $ctx0.prev = 2;
        $ctx0.next = 5;
        return fs.readlink.bind(null, out);
      case 5:
        link = $ctx0.sent;
      ...
      case 10:
        $ctx0.prev = 10;
        $ctx0.t1 = $ctx0.catch(2);
      case 12:
        $ctx0.next = 14;
        return mkdirp.bind(null, dirname(out));
      case 14:
        return $ctx0.delegateYield(utils.unlink(out), "t2", 15);
      case 15:
        $ctx0.next = 17;
        return fs.symlink.bind(null, file.filename, out);
      case 17:
      case "end":
        return $ctx0.stop();
      }
